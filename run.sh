...
certfile=$(jq -r '.certfile // "/ssl/fullchain.crt"' "${OPTIONS}")
keyfile=$(jq -r '.keyfile // "/ssl/privkey.pem"' "${OPTIONS}")
...
if [ -n "${domain}" ]; then
    ...
    if [ "${ssl}" = "true" ]; then
        ...
        export FORGEJO__server__CERT_FILE="${certfile}"
        export FORGEJO__server__KEY_FILE="${keyfile}"
    else
